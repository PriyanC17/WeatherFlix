...
def get_weather_data1(city, predictions, weather):
    ...
    weather_data = {
        'city':city,
        'wind': wind,
        'temperature': "{:.2f}".format(temperature),  # Removed as not using predictions
        'humidity': humidity,
        'date': datetime.utcnow() + timedelta(hours=5, minutes=30),
        # 'date':current_time,
        'visibility': visibility,
...
# def weather_for_5_days(city, weather):
#     forecast_data = []
#     for _ in range(5):  # Assuming 5-day forecast, adjust as needed
#         forecast_data.append({
#             'wind': weather.loc[city,'target_wind_mph'],
#             # 'temperature': prediction.get('target_temperature_celsius'),  # Removed as not using predictions
#             'humidity': weather.loc[city,'target_humidity'],
#             # 'icon': prediction.get('icon'),  # Removed as not using predictions
#             'visibility': weather.loc[city,'target_visibility_km'],
#             # 'description': weather['target_condition_text'],  # Removed as not using predictions
#             'feels_like': weather.loc[city,'target_feels_like_celsius']
#         })
#     return forecast_data
...
def prediction():
    if current_user.is_authenticated:
        ...
        if weather_data:
            ...
            flash('Weather information not available for this city.', 'warning')
            return redirect(url_for('home'))
# weather = pd.read_csv("C:\\Users\\abc\\Desktop\\WeatherFlix\\myenv\\WFlix\\project\\IndianWeatherRepository_2.csv",
#                               index_col="location_name")  # Assuming you're reloading weather data here
# @app.route("/weather_forecast")
# @login_required
# def weather_forecast():
#     if current_user.is_authenticated:
#         user_city = current_user.city
#         weather_predictions = backtest(weather, rr, predictors, start=3650, step=90)  # Obtain predictions if needed (for future enhancements)
#         # Convert 'last_updated' to numeric (assuming it's a datetime column)
#         # weather['last_updated'] = pd.to_datetime(weather['last_updated'], format='%d-%m-%y', dayfirst=True)
#         # weather.last_updated = pd.to_numeric(weather.last_updated)
#
#         weather_data = get_weather_data(user_city, weather)
#         weather_for_5 = weather_for_5_days(user_city, weather)
#         if weather_data and weather_for_5:
#             # Get the current time
#             current_time = datetime.utcnow() + timedelta(hours=5, minutes=30)
#
#             next_5_days_forecast = weather_for_5  # Assuming no need to slice (adjust as needed)
#             print(next_5_days_forecast)
#             return render_template('weather_data.html', weather_data=weather_data,
#                                    weather_for_5=next_5_days_forecast)
#         else:
#             flash('Weather information not available for this city.', 'warning')
#             return redirect(url_for('home'))
